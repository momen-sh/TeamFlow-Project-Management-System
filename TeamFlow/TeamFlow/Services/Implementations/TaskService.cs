...
namespace TeamFlow.Services.Implementations
{
    public class TaskService : ITaskService
    {
        ...
        {
            // Use a no-tracking read to validate existence and membership, then load a tracked entity
            var taskRead = await _taskRepository.GetWithProjectMembersAsync(taskId);
            if (taskRead is null)
                return ServiceResult<TaskItem>.Failure("Task not found");

            if (taskRead.AssignedUserId.HasValue)
                return ServiceResult<TaskItem>.Failure("Task is already assigned");

            if (!await _projectRepository.IsUserAssignedToProjectAsync(taskRead.ProjectId, currentUserId))
                return ServiceResult<TaskItem>.Failure("You must belong to the project to self-assign this task");

            // Load tracked entity to avoid updating the entire graph (navigation properties) when saving
            var tracked = await _taskRepository.FindAsync(taskId);
            if (tracked is null)
                return ServiceResult<TaskItem>.Failure("Task not found");

            tracked.AssignedUserId = currentUserId;
            await _taskRepository.SaveAsync();

            return ServiceResult<TaskItem>.Success(tracked);
        }
